package app.linuxdeploy.receiver;
...
import app.linuxdeploy.EnvUtils;
import app.linuxdeploy.R;
import app.linuxdeploy.activity.MainActivity;

import static app.linuxdeploy.App.SERVICE_CHANNEL_ID;
...
public class ActionReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, Intent intent) {
        // am broadcast -a app.linuxdeploy.BROADCAST_ACTION --user 0 --esn "hide"
        if (intent.hasExtra("hide")) {
            hideNotification(context);
            return;
        }
        // am broadcast -a app.linuxdeploy.BROADCAST_ACTION --user 0 --es "info" "Hello World!"
        if (intent.hasExtra("info")) {
            showNotification(context, android.R.drawable.ic_dialog_info, intent.getStringExtra("info"));
            return;
        }
        // am broadcast -a app.linuxdeploy.BROADCAST_ACTION --user 0 --es "alert" "Hello World!"
        if (intent.hasExtra("alert")) {
            showNotification(context, android.R.drawable.ic_dialog_alert, intent.getStringExtra("alert"));
            return;
        }
        // am broadcast -a app.linuxdeploy.BROADCAST_ACTION --user 0 --esn "start"
        if (intent.hasExtra("start")) {
            System.out.println("START");
            EnvUtils.execService(context, "start", "-m");
            return;
        }
        // am broadcast -a app.linuxdeploy.BROADCAST_ACTION --user 0 --esn "stop"
        if (intent.hasExtra("stop")) {
            System.out.println("STOP");
            EnvUtils.execService(context, "stop", "-u");
            return;
        }
        // am broadcast -a app.linuxdeploy.BROADCAST_ACTION --user 0 --esn "show"
